...
export class ChauffeurService {
  public resourceUrl = SERVER_API_URL + 'services/transportdb/api/chauffeurs';
  ...
  protected convertDateFromClient(chauffeur: IChauffeur): IChauffeur {
    const copy: IChauffeur = Object.assign({}, chauffeur, {
      dateNaiss: chauffeur.dateNaiss && chauffeur.dateNaiss.isValid() ? chauffeur.dateNaiss.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((chauffeur: IChauffeur) => {
        chauffeur.dateNaiss = chauffeur.dateNaiss ? moment(chauffeur.dateNaiss) : undefined;
      });
    }
    return res;
  }
