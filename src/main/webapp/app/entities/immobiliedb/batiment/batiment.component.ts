...
import { IBatiment } from 'app/shared/model/immobiliedb/batiment.model';
...
export class BatimentComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.batimentService
      ...
      .subscribe(
        (res: HttpResponse<IBatiment[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInBatiments(): void {
    this.eventSubscriber = this.eventManager.subscribe('batimentListModification', () => this.loadPage());
  }
